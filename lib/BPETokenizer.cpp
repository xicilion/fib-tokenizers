...
napi_value BPETokenizer::from(napi_env env, napi_callback_info info)
{
    ...
    NODE_API_CALL(env, napi_typeof(env, args[2], &valuetype0));
    NODE_API_ASSERT(env, valuetype0 == napi_object, "Wrong argument type, must be object");

    napi_value keys;
    NODE_API_CALL(env, napi_get_property_names(env, args[1], &keys));
    ...
    try {
        ...
        NODE_API_CALL(env, napi_get_array_length(env, args[2], &merge_length));

        for (int merge_count = 1; merge_count < merge_length; merge_count++) {
            napi_value e;
            NODE_API_CALL(env, napi_get_element(env, args[2], merge_count, &e));
